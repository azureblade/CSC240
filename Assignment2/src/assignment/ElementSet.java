 ...
 * Assignment #2
 * 
 * @author Dr. Richard Epstein, Matthew Wallace
 */
...
public class ElementSet {
	...
	public boolean displayAnObject(Element anObject) {
		for (int i = 0; i < currentSize; i++) {
			...
			if (className.equals("Resort")) {
				if (((Resort) e).equals(anObject)) {
					e.display();
					return true;
				}
			} else if (className.equals("Airline")) {
				if (((Airline) e).equals(anObject)) {
					e.display();
					return true;
				}
			}
	...
	public boolean removeAnObject(Element editedObject) {
		String className = editedObject.getClassName();
		String currentClass;
		Element last;
